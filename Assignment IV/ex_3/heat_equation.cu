...
int main(int argc, char **argv) {
    ...
    cusparseDnVecDescr_t vecTemp;     // Array to store the final time step
    cusparseDnVecDescr_t vecTmp;      // Temporal array of dimX for computations
    ...
    cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, opA, &one, Adescriptor,
    ...
    for (int it = 0; it < nsteps; ++it) {
        ...
        //@@ tmp = 1 * A * temp + 0 * tmp
        cputimer_start();
        cusparseCheck(cusparseSpMV(cusparseHandle, opA, &one, Adescriptor,
                                   vecTemp, &zero, vecTmp, CUDA_R_64F,
    ...
    printf("Timing - SpMV execution time. \t\tElasped %.0f microseconds \n", spmvTime);
